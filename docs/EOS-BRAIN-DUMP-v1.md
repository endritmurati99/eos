...
Fuer jeden Brain Dump wird zuerst genau eine `brain_dump_note` unter `10 Inbox/YYYY-MM-DD-HHmm-brain-dump.md` angelegt.
...
Eine `brain_dump_note` bleibt in `10 Inbox`, solange die Triage nicht abgeschlossen ist.
...
Ziel:
- `10 Inbox` bleibt ein echter Capture-Puffer
