...
### Erwartete Semantik
- `list_structure()` prueft die realen Google-Tasks-Listen gegen `Inbox`, `Next`, `Waiting`, `This Week`
...
### Stabile Statuswerte
Read- und Mutationspfade muessen die folgenden Statuswerte stabil halten:
- `success`
- `partial`
- `auth_required`
- `config_missing`
...
`403 accessNotConfigured`, `SERVICE_DISABLED` und sinngleiche Google-API-Disablement-Meldungen werden als `provider_disabled` behandelt. Das bedeutet produktseitig: Google Tasks ist gerade nicht als verifizierte Aufgabenbasis nutzbar, und EOS darf nicht auf lokale Stub-Aufgaben ausweichen.
...
- fehlende Listen duerfen erst nach erfolgreichem Read und explizitem Nicht-Dry-Run angelegt werden
...
- markiere den Zustand explizit als `auth_required`, `config_missing`, `provider_disabled`, `not_found` oder `provider_error`
- nenne klar, dass die Aufgabenbasis derzeit nicht verfuegbar ist
- erfinde keine Prioritaeten
- erfinde keine Top-3
- verwende `data/tasks.json` niemals als Produktionsersatz
...
- keine lokale JSON-Datei als Ersatz fuer einen fehlgeschlagenen Live-Read in Produktionsbriefings
