...
- `bash scripts/run_eos_cli_smoke.sh`: success exit through privacy-safe summaries. CLI help works; health returns `warning`; `cron-audit` and `model-audit` return `success`; daily-plan dry-run returns `yellow`; weekly-plan dry-run returns `success`.
- `bash scripts/run_eos_cron_smoke.sh`: success exit through privacy-safe summaries. Cron audit succeeds; Systemd timer checks are summarized without raw timer output.
...
- Readonly DB: `attempt to write a readonly database` is classified as `readonly_database`.
...
The smoke scripts now suppress raw command output and emit only safe summaries. Calendar/task dry-run output can contain live personal data, so CI and external reports must use the summary lines only.
...
- Full CLI dry-run output can include live personal calendar/task data. Smoke scripts must continue to suppress raw output and report status/error classes only.
