...
Status: Phase 1 read-only readiness notes. Google Cloud UI steps and live Gmail E2E are not verified by this repository change.
...
```bash
python3 -m src.eos_cli mail auth-check --dry-run --json-only
```

Only after OAuth setup is reviewed, run a bounded read-only audit:

```bash
python3 -m src.eos_cli --json-only mail audit --last 1d --dry-run --limit 1
```

Live Gmail E2E remains unverified until this succeeds with real credentials and no secret output.
