...
from scripts import eos_safe_smoke_summary
...
def test_safe_smoke_classifies_readonly_database_without_raw_error() -> None:
    raw_error = "sqlite3.OperationalError: attempt to write a readonly database"

    summary = eos_safe_smoke_summary.summarize_command("daily_plan", 1, "", raw_error)
...
def test_safe_smoke_detects_sensitive_raw_output() -> None:
    raw_output = json.dumps(
        {
            ...
            "thread_id": "thread_abc123456789",
        }
    )

    assert eos_safe_smoke_summary.contains_sensitive_output(raw_output)


def test_safe_smoke_masks_nested_values() -> None:
    payload = {
        "token": "raw-secret-value",
        "nested": ["person@example.com", {"chat": "telegram:123456789"}],
    }

    masked = eos_safe_smoke_summary.mask_value(payload)
    serialized = json.dumps(masked)

    assert "raw-secret-value" not in serialized
    assert "person@example.com" not in serialized
    assert "telegram:123456789" not in serialized


def test_safe_smoke_missing_gog_is_warning_class() -> None:
    summary = eos_safe_smoke_summary.summarize_command("gog_probe", 127, "", "gog: command not found")

    assert summary["status"] == "warning"
    assert summary["error_class"] == "missing_gog"
