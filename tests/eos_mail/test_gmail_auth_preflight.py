...
def test_preflight_does_not_crash_without_gog(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    assert any(issue["code"] == "gog_missing" for issue in result["issues"])
...
def test_preflight_missing_account_is_sanitized_config_missing(
    ...
    assert any(issue["code"] == "account_missing" for issue in result["issues"])
...
def test_mail_cli_auth_check_is_dry_run_only() -> None:
    dry_run_result = command_mail(
        argparse.Namespace(
            mail_command="auth-check",
            dry_run=True,
        )
    )
    blocked_result = command_mail(
        argparse.Namespace(
            mail_command="auth-check",
            dry_run=False,
        )
    ...
    assert dry_run_result["gmail_write_actions_added"] is False
    assert blocked_result["status"] == "failed"
    assert blocked_result["error_class"] == "write_mode_forbidden"
